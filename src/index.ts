import Client2 = require("./Client");
import Constants = require("./Constants");
import Intents = require("./Intents");

import Shard2 = require("./Shard");
import ShardManager2 = require("./ShardManager");

import BetterWs2 = require("./BetterWs");
import StateMachine2 = require("./StateMachine");
import DiscordConnector2 = require("./DiscordConnector");

export { BetterWs2 as BetterWs, StateMachine2 as StateMachine, DiscordConnector2 as DiscordConnector, Client2 as Client, Constants, Intents, Shard2 as Shard, ShardManager2 as ShardManager };
export * from "./Types";
